        ...
        if (value !== "") {
          const filteredArr = data.filter((result) => {
            return (
              ...
              result.email.includes(value) ||
              result.phone.includes(value)
            );
          });
    ...
    httpClient
      ...
      .then((serverResponse) => {
        ...
        if (i <= FriendArr.length) {
          i++;
          let removeCurrentFriends = data.filter(
            (item) => item._id !== FriendArr[i]._id
          );
            ...
            <div key={item._id} className="column is-one-third" id="blue">
              ...
              >
                <figure className="image is-square">
                  <img
                    className="is-rounded is-256x256"
                    src={item.image}
                    alt={item.name}
                  />
                </figure>
                ...
                <button
                  className="button is-fullwidth is-dark is-medium addBtn"
                  id="friend"
